...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_product'])) {
        ...
        try {
            ...
            if (strpos($category, 'access_') === 0) {
                // For all accessories, only add stock
                $stock = $_POST['stock'][0];
                $stmt = $db->prepare("INSERT INTO product_variations (product_id, size, stock) VALUES (?, 'N/A', ?)");
                $stmt->execute([$product_id, $stock]);
            } else {
                // For shoes and clothing, add size variations
                $sizes = $_POST['size'];
                $stocks = $_POST['stock'];
                $price_adjustments = $_POST['price_adjustment'];
                
                for ($i = 0; $i < count($sizes); $i++) {
                    $stmt = $db->prepare("INSERT INTO product_variations (product_id, size, stock, price_adjustment) VALUES (?, ?, ?, ?)");
                    $stmt->execute([$product_id, $sizes[$i], $stocks[$i], $price_adjustments[$i]]);
                }
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('add-size-variation').addEventListener('click', function() {
                ...
                if (category.startsWith('access_')) {
                    // For all accessories, only add stock input
                    const newRow = document.createElement('div');
                    newRow.className = 'stock-variation';
                    newRow.innerHTML = `
                        <input type="number" name="stock[]" min="0" placeholder="Stock" class="form-control" required>
                        <button type="button" class="btn btn-danger remove-size"><i class="fas fa-times"></i></button>
                    `;
                    ...
                    // For clothing, add default size row
                    const newRow = document.createElement('div');
                    newRow.className = 'size-variation';
                    newRow.innerHTML = `
                        <select name="size[]" class="form-control">
            ...
            document.getElementById('category').addEventListener('change', function() {
                const container = document.querySelector('.size-variations-container');
                const addSizeButton = document.getElementById('add-size-variation');
                container.innerHTML = ''; // Clear existing size variations
                
                if (this.value.startsWith('access_')) {
                    // For all accessories, only show stock input
                    const newRow = document.createElement('div');
                    newRow.className = 'stock-variation';
                    newRow.innerHTML = `
                        <input type="number" name="stock[]" min="0" placeholder="Stock" class="form-control" required>
                    ...
                    // Hide the "Add Another Size" button for accessories
                    if (addSizeButton) {
                        addSizeButton.style.display = 'none';
                    }
                } else {
                    // For shoes and clothing, show the "Add Another Size" button
                    if (addSizeButton) {
                        addSizeButton.style.display = 'inline-block';
                    }
