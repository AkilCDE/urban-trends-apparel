...
class OrderDelivery {
    ...
    public function __construct($db) {
        ...
        $this->mailer->Username = 'trendwears.ubantrends@gmail.com'; // Change to your email
        $this->mailer->Password = 'zrcd gfna uadf gvzz'; // Change to your app password
    ...
    public function updateOrderStatus($order_id, $status, $notes = null) {
        try {
            ...
            $stmt = $this->db->prepare("
                SELECT u.email, u.phone, CONCAT(u.firstname, ' ', u.lastname) as fullname, 
                ...
                WHERE o.order_id = ?
                GROUP BY o.order_id, u.email, u.phone, u.firstname, u.lastname
            ");
            $stmt->execute([$order_id]);
            ...
            if ($order) {
                ...
                $message = "Your order #" . $order['order_number'] . " (" . $order['product_names'] . ") has been updated to: " . ucwords($status);
                
                // Send notifications via email and SMS with notes
                $this->notifications->sendOrderStatusNotification(
                    ...
                    $status,
                    $notes,
                    $order['product_names']  // Add product names as additional parameter
                );
            }
    ...
    private function sendStatusNotification($email, $name, $order_number, $status) {
        try {
            ...
            if ($mail_sent) {
                ...
                $stmt->execute([$email, $subject, $message, $error]);
                return false;
            }
        } catch (Exception $e) {
            error_log("Exception in sendStatusNotification: " . $e->getMessage());
            return false;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (isset($_POST['update_status'])) {
            ...
            // Get customer details before updating status
            $stmt = $db->prepare("
                SELECT o.*, u.email, u.phone, u.firstname, u.lastname 
                FROM orders o 
                JOIN users u ON o.user_id = u.user_id 
                WHERE o.order_id = ?
            ");
            ...
            if ($orderDelivery->updateOrderStatus($order_id, $status, $notes)) {
                ...
                $_SESSION['success_message'] = "Order status updated successfully! Notifications sent to customer.";
            }
        }
          ...
          <?php if (isset($order)): ?>
                ...
                <div class="status-update">
                    ...
                    <form method="POST" id="statusUpdateForm">
                        ...
                        <div class="form-group">
                            <label for="notes">Notes to Customer (optional)</label>
                            <textarea id="notes" name="notes" class="form-control" rows="4" 
                                    placeholder="Add any notes or instructions for the customer..."
                                    style="resize: vertical; min-height: 100px;"><?php echo isset($_POST['notes']) ? htmlspecialchars($_POST['notes']) : ''; ?></textarea>
                            <small class="form-text text-muted">
                                These notes will be sent to the customer via email and SMS notification.
                            </small>
                        </div>
                        
                        <button type="submit" name="update_status" class="btn btn-primary" onclick="return confirmStatusUpdate();">
                            <i class="fas fa-save"></i> Update Status & Notify Customer
                        </button>
                    </form>
                </div>
                ...
                function confirmStatusUpdate() {
                    ...
                    let message = `Are you sure you want to update the order status to "${status}"?`;
                    if (notes.trim()) {
                        message += `\n\nThe following note will be sent to the customer:\n${notes}`;
                    }
